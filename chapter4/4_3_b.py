...
def plot_2d(x,y,N,plot_id):
    ...
    plt.title(r'$\theta_n$'+" V.S. " +r'$n$'+' when N = '+str(N)+", with "+r'$\epsilon = 0.001$')
    plt.xlabel(r'$n$')
...
def plot_2d_heart_rate(x,y,N,plot_id):
    #array_=np.asarray(list_)
    plt.scatter(x,y,alpha = 0.03)
    plt.title('Expected Heart Rate'+" V.S. " +r'$n$'+' when N = '+str(N)+", with "+r'$\epsilon = 0.001$')
    plt.xlabel(r'$n$')
...
if __name__=="__main__":
    ...
    theta_n[0] = 0.5
    heart_beat_n[0] = 0
    learning_rate = 0.001*np.ones((TOTAL_NUMBER,))
    N = [1,10,20]
