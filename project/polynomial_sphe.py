...
def get_d_j(x):
    return 8*math.cos(x)**7*math.sin(x)-8*math.cos(x)**3*math.sin(x)+2*math.cos(x)*math.sin(x)
...
if __name__=='__main__':
   ...
   # print 'res',min[math.cos(_) for _ in estimated_theta]
    plot_new(list(range(len(res))),res,"Resuls of the spherical coordinates parameterization",'polynomial_est_sphe.eps',r'$n$',r'$\theta_n$')
    #plt.show()
